...
A [Claude Skill](https://docs.claude.com/en/docs/agents-and-tools/agent-skills/overview) for crafting production-grade web motion — hero entrances, scroll-triggered reveals, stagger/choreography, micro-interactions, Aceternity/Magic UI-style effects, page transitions, and scrollytelling — using plain CSS, native scroll-driven CSS, GSAP, or Motion (formerly Framer Motion) depending on what the moment actually needs.
...
- **`references/`** — loaded as needed, not all at once: animation principles, a tool decision framework, CSS-only patterns, GSAP patterns, Motion patterns, an Aceternity/Magic UI-style effects-to-technique catalog, performance rules, and accessibility rules.
- **`snippets/`** — copy-paste starting points (CSS: stagger, scroll-reveal, marquee, text-mask reveal, easing tokens, magnetic hover button, spotlight, aurora background, meteors; vanilla JS: 3D card tilt; Motion: shared-layout transition, scroll parallax; GSAP: hero timeline, scroll-trigger reveal, pinned scroll section, SVG path draw, a React cleanup pattern, Lenis sync).
